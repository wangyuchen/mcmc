  ...
  !            i2 = total(2)
  !
  !            l1 = gammln(dble(k(1)+i0))+gammln(dble(k(2)+i1))+ &
  !                  gammln(dble(k(3)+i2))-gammln(dble(sum(k)+i0+i1+i2))
  !
  !            l2 = gammln(dble(sum(k)))-gammln(dble(k(1)))-&
	! 		        gammln(dble(k(2)))-gammln(dble(k(3)))
  !
  !            prior = l1 + l2
  !        end function prior

!
! !  The function gammln returns the value ln[Gamma(xx)] for xx>0
!           function gammln(xx)
!            REAL(kind=8) gammln
!            integer j
!            DOUBLE PRECISION ser, stp, tmp, x,y, cof(6),xx
